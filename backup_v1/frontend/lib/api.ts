...
// Document-aware AI streaming helper
export async function streamDocumentAI(
  prompt: string,
  documentContext: {
  ...
  try {
    const response = await fetch(`${API_URL}/api/groq/document-stream`, {
      method: 'POST',
      headers: withAuth({ 'Content-Type': 'application/json', Accept: 'text/plain' }),
      body: JSON.stringify({ prompt, document_context: documentContext, model }),
    });
    ...
    // Fallback to regular streaming if document-aware endpoint fails
    console.warn('Document-aware AI failed, falling back to regular AI:', err);
    return streamGroq(prompt, onDelta, model);
  }
}

// Streaming Groq helper (original - for non-document queries)
